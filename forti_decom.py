...
def pop_policies(pol_id_dict, pol_id_list):
    ...
    for pol_id in pol_id_list:
        popped_pols.update({pol_id: pol_id_dict.pop(pol_id)})

    # Return the remaining policies dictionary & the popped policies dictionary
    return pol_id_dict, popped_pols
...
def main():
    ...
    remain_pols, popped_pols = pop_policies(config_dict['pol_dict'], pol_id_list)

    # Record all destination objects and services for policies to be deleted
    pop_obj_dict = record_objects(popped_pols)

    # Record all destination objects and services for remaining policies
    remain_obj_dict = record_objects(remain_pols)
